...
public class BusSimulator {
	...
	/**
	 * 
	 * @return The number of time steps that have elapsed since the program
	 *         started (t=0)
	 */
	...
	/**
	 * Runs the simulation to termination, seeking input from the player at
	 * every timestep where there is a bus at a station, ready to accept
	 * commands.
	 * 
	...
	public final int run() {
		...
		while (passengers.size() > 0) {
			...
			if (busAvailable()) { // If all busses are in transit, there's
									// nothing to do.
				try {
			...
			if (time >= killTime) { // End the program if nobody has been
